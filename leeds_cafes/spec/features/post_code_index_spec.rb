...
RSpec.describe 'when vsiting the post_code index page', type: :feature do
  ...
  it 'displays correct statistics for the post code named' do
    ...
    expect(Restaurant.find_by_post_code('LS1').count).to eq(9)
    expect(Restaurant.total_chairs('LS1')).to eq(184)
    expect(Restaurant.total_places('LS1')).to eq(9)
    expect(Restaurant.chairs_pct('LS1')).to eq(56.79)
